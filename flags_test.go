...
func TestFlagParsing(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			fs := flags.NewFlagSet().AddFlags(tt.flags...)
...
func TestPersistentFlagCollisions(t *testing.T) {
	...
	t.Run("local flag shadows persistent flag in ctx.Flags()", func(t *testing.T) {
		...
		var got string
		root := &gommand.Command{Name: "root", PersistentFlagSet: pfs}
		sub := &gommand.Command{
			Name:    "sub",
			FlagSet: localFS,
			Run:     func(ctx *gommand.Context) error { got = ctx.Flags().String("output"); return nil },
		}
		root.SubCommand(sub)
